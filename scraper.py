...
class DataMaker:
    ...
    def scrape_features(self, page_url):
       ...
       # print(course_list_soup)

        cnames = course_list_soup.select(".card-title")
        print(cnames)
        for i in range(min(len(cnames), 10)):
            self.courses.append(cnames[i].text)
        
        pnames = course_list_soup.select(".horizontal-box > .partner-name")
        for i in range(min(len(pnames), 10)):
...
def main():
    ...
    df = dm.make_dataset()
    destination_path = os.path.join(r"C:/Users/hansi/Desktop/Projects/Optshine_Course_Recommender/coursera-courses-overview.csv")
